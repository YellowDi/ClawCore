...
const (
	// 以下消息类型组成 ClawBridge、ClawCore 和当前模拟 OpenClaw 客户端之间的轻量线协议。
...
// OpenClawMessage 表示 OpenClaw 侧的双向消息帧：user.message 发给 OpenClaw，assistant.message 返回给 ClawCore。
...
// ErrorMessage 表示 ClawCore 发送给浏览器或 OpenClaw 客户端的错误帧。
...
// DeliveryResponse 是 OpenClaw HTTP 回传接口的投递结果。
