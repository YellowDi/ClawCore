...
import {
  createClawBridgeMessageReceipt,
  sendClawBridgeText,
} from "./outbound.js";
import {
  looksLikeClawBridgeTarget,
  normalizeClawBridgeTarget,
} from "./target.js";
...
export const clawBridgePlugin: ChannelPlugin<ResolvedClawBridgeAccount> =
  createChatChannelPlugin({
    base: {
      id: CHANNEL_ID,
      meta: {
      ...
      config: {
        listAccountIds: (cfg) => listClawBridgeAccountIds(cfg as ClawBridgeCoreConfig),
        resolveAccount: (cfg, accountId) =>
          resolveClawBridgeAccount({
            cfg: cfg as ClawBridgeCoreConfig,
            accountId,
          }),
        defaultAccountId: (cfg) =>
          resolveDefaultClawBridgeAccountId(cfg as ClawBridgeCoreConfig),
    ...
    outbound: {
      ...
      attachedResults: {
        channel: CHANNEL_ID,
        sendText: async ({ cfg, accountId, to, text, replyToId }) =>
          await sendClawBridgeText({
            cfg,
            accountId,
            to,
            text,
